...
On 7 September 2026, the full Azure inventory was reviewed and deletion completed for the old website, B1 App Service plan, SQL server/database, private endpoint, virtual network, DNS zone, network security group, Log Analytics workspace and Network Watcher. These were the only resources in the subscription. Follow-up inventory confirmed zero resources and zero resource groups. No SQL long-term-retention backups remain.

After explicit user authorization, Azure accepted cancellation of the empty free-trial subscription on 7 September 2026. The cancellation receipt is saved privately alongside the backup. The immediate subscription read showed `Warned` rather than `Enabled`; Azure can take up to 10 minutes to reflect cancellation in its portal. Microsoft states that billing stops immediately after cancellation. There is no remaining Azure-hosted website or provisioned resource. See [Azure cancellation and billing](https://learn.microsoft.com/en-us/azure/cost-management-billing/manage/cancel-azure-subscription).
...
The old `sam-speed.azurewebsites.net` address cannot be transferred to Cloudflare and is retired. Use https://sam-speed-models.pages.dev. Hosting and updates use the free services described above. Deletion does not erase any charges already accrued before retirement.
